...
public abstract class AbstractContainerScreen<T extends AbstractContainerMenu> extends Screen implements MenuAccess<T> {
    ...
    private void extractFloatingItem(GuiGraphicsExtractor graphics, ItemStack carried, int x, int y, @Nullable String itemCount) {
        ...
        graphics.itemDecorations(font != null ? font : this.font, carried, x, y, itemCount);
    }
    ...
    public boolean mouseClicked(MouseButtonEvent event, boolean doubleClick) {
        ...
        if (event.button() != 0 && event.button() != 1 && !cloning) {
            ...
            if (slot != null) clickedOutside = false; // Neo: prevent dropping of items through slots outside of GUI boundaries
            int slotId = -1;
            if (slot != null) {
                slotId = slot.index;
            }
