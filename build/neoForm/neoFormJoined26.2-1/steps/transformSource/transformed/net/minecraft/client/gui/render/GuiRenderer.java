...
public class GuiRenderer implements AutoCloseable {
    ...
    public GuiRenderer(GuiRenderState renderState, FeatureRenderDispatcher featureRenderDispatcher, List<net.neoforged.neoforge.client.gui.PictureInPictureRendererRegistration<?>> pipRendererFactories) {
        this.renderState = renderState;
        this.featureRenderDispatcher = featureRenderDispatcher;
        this.pictureInPictureRendererPools = net.neoforged.neoforge.client.gui.PictureInPictureRendererPool.createPools(pipRendererFactories);
    }
    ...
    private void addElementToMesh(GuiElementRenderState elementState) {
        ...
        if (this.previousDraw == null
            ...
            || pipeline.getPrimitiveTopology().connectedPrimitives // Neo: flush elements with connected primitives individually
    ...
    private void preparePictureInPicture() {
        ...
        this.renderState.forEachPictureInPicture(state -> {
            if (this.preparePictureInPictureState(state, guiScale, true)) {
                this.pictureInPictureRenderStatesScratch.add(state);
            }
        });
        this.renderState.forEachPictureInPicture(state -> {
            if (this.pictureInPictureRenderStatesScratch.add(state)) {
                this.preparePictureInPictureState(state, guiScale, false);
            }
        });
        this.pictureInPictureRenderStatesScratch.clear();
    }
    ...
    private <T extends PictureInPictureRenderState> boolean preparePictureInPictureState(T picturesInPictureState, int guiScale, boolean firstPass) {
        var pool = (net.neoforged.neoforge.client.gui.PictureInPictureRendererPool<T>)this.pictureInPictureRendererPools.get(picturesInPictureState.getClass());
        if (pool == null) {
            return false;
        }
        PictureInPictureRenderer<T> renderer = pool.get(picturesInPictureState, guiScale, firstPass);
        if (renderer != null) {
    ...
    private void executeDraw(GuiRenderer.Draw draw, RenderPass renderPass) {
