...
public class ItemInHandRenderer {
    ...
    private void renderMap(PoseStack poseStack, SubmitNodeCollector submitNodeCollector, int lightCoords, ItemStack itemStack) {
        ...
        MapItemSavedData data = MapItem.getSavedData(itemStack, this.minecraft.level);
    ...
    public void submitHandsWithItems(float frameInterp, PoseStack poseStack, SubmitNodeCollector submitNodeCollector, LocalPlayer player, int lightCoords) {
        ...
        if (handRenderSelection.renderMainHand) {
            ...
            if(!net.neoforged.neoforge.client.ClientHooks.renderSpecificFirstPersonHand(InteractionHand.MAIN_HAND, poseStack, submitNodeCollector, lightCoords, frameInterp, xRot, mainHandAttack, mainhandInverseArmHeight, this.mainHandItem))
        ...
        if (handRenderSelection.renderOffHand) {
            ...
            if(!net.neoforged.neoforge.client.ClientHooks.renderSpecificFirstPersonHand(InteractionHand.OFF_HAND, poseStack, submitNodeCollector, lightCoords, frameInterp, xRot, offHandAttack, offhandInverseArmHeight, this.offHandItem))
            this.submitArmWithItem(
                player,
                frameInterp,
                xRot,
    ...
    private void submitArmWithItem(
        ...
        if (!player.isScoping()) {
            ...
            if (itemStack.isEmpty()) {
                ...
                if (!net.neoforged.neoforge.client.extensions.common.IClientItemExtensions.of(itemStack).applyForgeHandTransform(poseStack, minecraft.player, arm, itemStack, frameInterp, inverseArmHeight, attack))
    ...
    public void tick() {
        ...
        if (player.isHandsBusy()) {
            ...
            boolean swapAnimMain = net.neoforged.neoforge.client.ClientHooks.shouldCauseReequipAnimation(this.mainHandItem, nextMainHand, player.getInventory().getSelectedSlot());
            boolean swapAnimOff = net.neoforged.neoforge.client.ClientHooks.shouldCauseReequipAnimation(this.offHandItem, nextOffHand, -1);

            if (!swapAnimMain && this.mainHandItem != nextMainHand)
                this.mainHandItem = nextMainHand;
            if (!swapAnimOff && this.offHandItem != nextOffHand)
                this.offHandItem = nextOffHand;

            float attackAnim = player.getItemSwapScale(1.0F);
            float mainHandTargetHeight = swapAnimMain ? 0.0F : attackAnim * attackAnim * attackAnim;
