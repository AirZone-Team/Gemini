...
public class Hud {
    ...
    private void registerVanillaLayers() {
        java.util.function.BooleanSupplier hudVisible = () -> !this.isHidden;
        java.util.function.BooleanSupplier survivalVisible = () -> this.minecraft.gameMode.canHurtPlayer() && hudVisible.getAsBoolean();

        layerManager.add(CAMERA_OVERLAYS, this::extractCameraOverlays, hudVisible);
        layerManager.add(CROSSHAIR, this::extractCrosshair, hudVisible);
        layerManager.add(AFTER_CAMERA_DECORATIONS, (guiGraphics, _) -> guiGraphics.nextStratum(), hudVisible);
        // TODO: Split this up again into its pieces
        layerManager.add(HOTBAR, this::extractHotbar, hudVisible);
        layerManager.add(PLAYER_HEALTH, (guiGraphics, _) -> extractHealthLevel(guiGraphics), survivalVisible);
        layerManager.add(ARMOR_LEVEL, (guiGraphics, _) -> extractArmorLevel(guiGraphics), survivalVisible);
        layerManager.add(FOOD_LEVEL, (guiGraphics, _) -> extractFoodLevel(guiGraphics), survivalVisible);
        layerManager.add(VEHICLE_HEALTH, (guiGraphics, _) -> extractVehicleHealth(guiGraphics), hudVisible);
        layerManager.add(AIR_LEVEL, (guiGraphics, _) -> extractAirLevel(guiGraphics), survivalVisible);
        layerManager.add(CONTEXTUAL_INFO_BAR_BACKGROUND, this::extractContextualInfoBarBackground, hudVisible);
        layerManager.add(EXPERIENCE_LEVEL, this::extractExperienceLevel, hudVisible);
        layerManager.add(CONTEXTUAL_INFO_BAR, this::extractContextualInfoBar, hudVisible);
        layerManager.add(SELECTED_ITEM_NAME, this::maybeExtractSelectedItemName, hudVisible);
        layerManager.add(SPECTATOR_TOOLTIP, this::maybeExtractSpectatorTooltip, hudVisible);
        layerManager.add(EFFECTS, this::extractEffects, hudVisible);
        layerManager.add(BOSS_OVERLAY, this::extractBossOverlay, hudVisible);
        layerManager.add(SLEEP_OVERLAY, this::extractSleepOverlay);
        layerManager.add(DEMO_OVERLAY, this::extractDemoOverlay, hudVisible);
        layerManager.add(SCOREBOARD_SIDEBAR, this::extractScoreboardSidebar, hudVisible);
        layerManager.add(OVERLAY_MESSAGE, this::extractOverlayMessage, hudVisible);
        layerManager.add(TITLE, this::extractTitle, hudVisible);
        layerManager.add(CHAT, this::extractChat, hudVisible);
        layerManager.add(TAB_LIST, this::extractTabList, hudVisible);
    ...
    private void extractSelectedItemName(GuiGraphicsExtractor graphics, int yShift) {
        if (this.toolHighlightTimer > 0 && !this.lastToolHighlight.isEmpty()) {
            ...
            if (alpha > 0) {
                ...
                if (font == null) {
                graphics.textWithBackdrop(this.getFont(), highlightTip, x, y, strWidth, ARGB.white(alpha));
                } else {
    ...
    private void displayScoreboardSidebar(GuiGraphicsExtractor graphics, Objective objective) {
        ...
        graphics.text(this.getFont(), objectiveDisplayName, left + width / 2 - objectiveDisplayNameWidth / 2, headerY - 9, -1, false);
    ...
    private void extractHeart(GuiGraphicsExtractor graphics, Hud.HeartType type, int xo, int yo, boolean isHardcore, boolean blinks, boolean half) {
    ...
    public enum HeartType implements IExtensibleEnum {
        ...
        private static Hud.HeartType forPlayer(Player player) {
            Hud.HeartType type;
        ...
        public static net.neoforged.fml.common.asm.enumextension.ExtensionInfo getExtensionInfo() {
            return net.neoforged.fml.common.asm.enumextension.ExtensionInfo.nonExtended(Hud.HeartType.class);
        }
    }
